...
- **CLI Resolution**: Resolves a precompiled binary using `cmd-lite.cliPath` or PATH (`cmd` / `command-code`). Stale Node package entrypoints such as `dist/index.mjs` are ignored.
- **Dependencies**: Uses `esbuild` for bundling and `@vscode/vsce` for packaging.

## Installation Methods
To install and run this extension:
1. **Prerequisite**: Install the precompiled `command-code` CLI and ensure `cmd` or `command-code` is on PATH:
   ```bash
   cmd --version
   ```
...
- `pnpm run watch` for auto-compilation during development.
- `pnpm run typecheck` to run typescript checks.
...
- **Problem**: Multiple package managers can register different `cmd` binaries, leaving the editor pointed at an older or stale executable.
- **Solution**: Check `which cmd` / `which command-code` and set `cmd-lite.cliPath` to the intended precompiled binary. Avoid editor-local Node package entrypoints like `globalStorage/.../cli/dist/index.mjs`.
  ```bash
  which cmd
  ```
...
- **Token Handshake Security**: The extension secures UDS connections by establishing a UUID token handshake. Session metadata is written with restricted permissions, auth tokens are no longer logged, and the remaining plaintext session-file token exists only for current CLI handshake compatibility.
...
## Windows Alias & Privacy Toggles
- **Windows CLI Name Collisions**: The hardcoded command name `cmd` conflicts with `cmd.exe` on Windows systems. By implementing a dynamic fallback (`process.platform === 'win32' ? 'cmdc' : 'cmd'`) in `src/cli/resolve.ts`, we avoid native command collisions on Windows while preserving the standard UX on POSIX systems.
- **Zero Data Retention (ZDR)**: Privacy toggles for subprocesses must propagate via strictly-defined environment variables. By checking the `cmd-lite.zeroDataRetention` boolean configuration and passing `CMD_ZDR="1"` down through `NodeJS.ProcessEnv`, we enforce clean execution boundaries without altering the CLI package's internal file persistence logic directly.

## Strict Modular Refactoring & Test Aggregation (v0.6.5)
- **Import Sanitation in Strict TypeScript**: When decomposing monolithic files to maintain the hard `<500 LOC` constraint, extracting blocks leaves dormant imports behind. In strict tsconfig setups with `noUnusedLocals`, `tsc --noEmit` will fail on unused variables. Surgical pruning of imports and keeping namespaces focused ensures clean compilation.
- **Modular Test Aggregation for Static Webview Assertions**: When monolithic UI files (such as `src/webview/main.ts`) are decomposed into cohesive modules (`input.ts`, `messages.ts`, `footer.ts`), regression tests that read the file from disk using `readFileSync` must aggregate all `.ts` files in the directory. Reading only `main.ts` will produce false negatives for components moved to sibling modules.
- **Chat Participant & Autocomplete Parity**: Commands supported in webview slash dispatch (`/design`, `/fix`) must be symmetrically registered in `package.json` under `contributes.chatParticipants[0].commands`, handled in `participant.ts` prompt construction, and populated with follow-up suggestions in `provideFollowups` to provide seamless autocomplete across both the native chat and sidebar webview interfaces.
- **Babashka Task Modularization**: When Babashka automation scripts like `scripts/dogfood.clj` exceed 500 lines due to large declarative test suites, separating tasks into a sibling namespace (`scripts/dogfood_tasks.clj`) and loading via `(load-file ...)` and `(refer 'dogfood-tasks)` preserves pure function boundaries while respecting file size limits.

## Rebranding: Decomplecting Brand from Technical Identity (v0.5.7)
- **Display Name vs Identifier Separation**: When rebranding a VS Code extension (CMD Lite → CommandCode+), user-facing display text (displayName, command titles, descriptions, error messages) is a separate concern from technical API identifiers (`cmd-lite.*` command IDs, view IDs, config keys). Changing identifiers breaks existing user `settings.json`, keybindings, and menu configurations. Only change display text; preserve all `cmd-lite.*` identifiers.
- **Upstream CLI References are Immutable**: The extension wraps the `Command Code` CLI — its name, `.commandcode/` paths, `commandcode.ai` URLs, and `commandcode-diff://` URI schemes are properties of the upstream product, not our brand. Never rename these during an extension rebrand.
- **Scope Discipline via grep-first**: Running `grep -r "CMD Lite"` across the project produces a precise manifest of every string to change. This prevents both under-replacement (stale brand text) and over-replacement (accidentally changing technical identifiers). Always verify with `grep` post-rebrand to confirm zero remaining references.

## Webview & Extension Host Decomposition and Test Isolation (v0.5.8)
- **Zero-Dependency Sub-Module Aggregation**: Decomposing a 2,496 LOC Vanilla JS/TS webview into 9 single-responsibility modules (`state`, `scroll`, `highlight`, `toast`, `footer`, `input`, `messages`, `panels`, `layout`) ensures each module does one thing well without framework bloat. `main.ts` orchestrates event registration and lifecycle in 463 LOC.
- **Extension Host Handler Separation**: Splitting `extension.ts` into `src/commands/registration.ts`, `src/handlers/chatInput.ts`, `src/handlers/webviewActions.ts`, and `src/lifecycle/integration.ts` reduces file size to 303 LOC, keeps command registration declarations cleanly separated from IPC socket lifecycles, and enforces the repository's `<500 LOC` constraint.
- **Vitest Multi-Worktree Test Contamination**: When git worktrees (e.g. `.kilo/worktrees/`) reside inside the repository workspace, Vitest's default project root discovery will scan and run test files across all worktree directories simultaneously. This causes file deletion race conditions (`ENOENT: unlink /tmp/...`) and missing artifact failures. Explicitly adding a root `vitest.config.ts` that includes only `src/` and excludes `**/.kilo/**` guarantees test isolation and zero flaky failures.
