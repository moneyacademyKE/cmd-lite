...
## Phase 4: Webview Extensibility (Current Focus)

Enhance the "Thin Glass" Webview renderer to natively consume richer JSON-RPC payloads generated by the orchestrating agent's MCP servers, without introducing local state entanglement.

### Objectives
- **Rich Visualization**: Render base64 tool outputs (e.g. `RenderImage` for Puppeteer screenshots) inline within the chat history.
- **Agent Dashboards**: Add dynamic UI panels to visualize concurrent sub-agents actively executing parallel tasks.
- **Interactive Diff Proposals**: Render agent code proposals as inline diff blocks with interactive "Accept/Reject" buttons, pushing the human-in-the-loop decisions natively to the extension.
- **Context Drag-and-Drop**: Implement Drag and Drop file ingestion. The Webview handles the browser interaction and delegates the DataURI bridging to the IDE via JSON-RPC.
